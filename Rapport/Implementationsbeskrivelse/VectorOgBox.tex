...
\begin{itemize}
	\item \itembf{translate(Box from, Box to)}: Translate tager to bokse og skalerer vektoren fra den ene boks, som den befinder sig i, til den anden boks.
	\item \itembf{mirrorY(Box box)}: mirrorY spejler vektoren i forhold til y-aksen i boksen.
	\item \itembf{isInside(Box box)}: isInside tager en boks som parameter og tjekker om vektoren befinder sig i boksen.
	\item \itembf{toArray()}: toArray returnere værdien af x og y for en vektor i en tabel (array).
\end{itemize}
...
\begin{itemize}
	\item \itembf{overlapping(Box box)}: Overlapping fortæller om boksen overlappes af en anden boks.
	\item \itembf{ratio()}: Boksens ratio returneres.
	\item \itembf{toArray()}: Boksens koordinater returneres i et to-dimensionelt array.
	\item \itembf{grow(double amount)}: Får boksen til at vokse i alle retninger.
\end{itemize}